...
namespace Blob.Tests
{
    public class TestBlobTree
    {
        ...
        void CompareTree(ref BlobTree<int> blobTree, IReadOnlyList<TreeNode<int>> tree)
        {
            CompareTree(ref blobTree, tree, node => node.Value / 100 - 1);
        }

        delegate int GetNodeIndex<T>(BlobTree<T>.Node node) where T : unmanaged;

        void CompareTree<T>(ref BlobTree<T> blobTree, IReadOnlyList<TreeNode<T>> tree, GetNodeIndex<T> getNodeIndexFunc) where T : unmanaged
        {
            for (var i = 0; i < blobTree.Length; i++)
            {
                var index = getNodeIndexFunc(blobTree[i]);
                tree[index].Index = i;
            }
            Assert.That(blobTree.Length, Is.EqualTo(tree.Count));
            for (var i = 0; i < blobTree.Length; i++)
            {
                var blobNode = blobTree[i];
                var node = tree[getNodeIndexFunc(blobNode)];
